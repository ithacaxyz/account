...
contract GuardedExecutorTest is SoladyTest {
    ...
    function testSpendERC20WithP256KeyViaEntryPoint() public {
        ...
        spendPermissionToken.mint(u.eoa, type(uint128).max);
        ...
        // Prep UserOp, and submit it. This UserOp should pass.
        {
            ...
            calls[0].target = address(spendPermissionToken);
            calls[0].data =
            ...
            // UserOp should pass.
            assertEq(ep.execute(abi.encode(u)), 0);
            assertEq(spendPermissionToken.balanceOf(address(0xb0b)), 0.6 ether);
        ...
        // Prep UserOp to try to exactly hit daily spend limit. This UserOp should pass.
        {
            ...
            assertEq(ep.execute(abi.encode(u)), 0);
            assertEq(spendPermissionToken.balanceOf(address(0xb0b)), 1 ether);
        ...
        assertEq(info.token, address(spendPermissionToken));
        ...
        // This UserOp should pass.
        {
            ...
            assertEq(ep.execute{gas: 20000000}(abi.encode(u)), 0);
            assertEq(spendPermissionToken.balanceOf(address(0xb0b)), 1.5 ether);
