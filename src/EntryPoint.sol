    ...
    ReentrancyGuardTransient
{
    ...
    bytes32 public constant USER_OP_TYPEHASH =
        keccak256(
    ...
    function _getEntryPointStorage()
        internal
        pure
        returns (EntryPointStorage storage $)
    {
    ...
    function execute(
    ...
    ) public payable virtual nonReentrant returns (bytes4 err) {
        ...
        assembly ("memory-safe") {
            ...
            if or(shr(64, t), lt(encodedUserOp.length, 0x20)) {
                revert(0x00, 0x00)
            }
        }
        ...
        assembly ("memory-safe") {
            ...
            if or(
                lt(shr(6, mul(gas(), 63)), add(g, _INNER_GAS_OVERHEAD)),
                shr(64, g)
            ) {
            ...
            calldatacopy(
                add(m, 0x20),
                encodedUserOp.offset,
                encodedUserOp.length
            )
            ...
            let gCapped := xor(
                g,
                mul(xor(g, _PAYMENT_GAS_CAP), lt(_PAYMENT_GAS_CAP, g))
            ) // `min`.
        ...
        uint256 finalPaymentAmount = Math.min(
            ...
            Math.saturatingMul(
                paymentPerGas,
                Math.saturatingAdd(gUsed, _REFUND_GAS)
            )
        );
    ...
    ) public payable virtual returns (bytes4[] memory errs) {
    ...
    function fill(
        ...
        if (orderId != bytes32(0)) {
            if (
                !_getEntryPointStorage().filledOrderIds.toggle(uint256(orderId))
            ) {
        ...
        assembly ("memory-safe") {
            ...
            if or(
                or(
                    shr(64, or(s, t)),
                    or(lt(originData.length, 0x60), lt(s, 0x60))
                ),
            ...
            eoa := calldataload(
                add(encodedUserOp.offset, calldataload(encodedUserOp.offset))
            )
        }
    ...
    function _pay(
        ...
        uint256 requiredBalanceAfter = Math.saturatingAdd(
            TokenTransferLib.balanceOf(paymentToken, address(this)),
            paymentAmount
        );
        address eoa = u.eoa;
    ...
    function _verify(
        ...
        assembly ("memory-safe") {
            ...
            isValid := and(
                eq(mload(0x00), 1),
                and(gt(returndatasize(), 0x3f), isValid)
            )
            keyHash := mload(0x20)
        }
    }
    ...
    function _execute(UserOp calldata u, bytes32 keyHash) internal virtual {
        ...
        assembly ("memory-safe") {
            if iszero(
                call(gas(), eoa, 0, add(0x20, data), mload(data), 0x00, 0x00)
            ) {
    ...
    function _computeDigest(
        ...
        unchecked {
            for (uint256 i; i != pointers.length; ++i) {
                (address target, uint256 value, bytes calldata data) = pointers
                    .getExecution(i);
                a.set(
                    i,
        ...
        assembly ("memory-safe") {
            ...
            if iszero(
                and(
                    ...
                    staticcall(gas(), eoa, 0x1c, 0x04, 0x00, 0x20)
                )
            ) {
                revert(0x00, 0x00)
            }
